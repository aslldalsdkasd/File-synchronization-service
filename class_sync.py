...
class Synchronization:
    ...
    def load(self, path):
        ...
        try:
            response = requests.get(get_url, headers=self.headers, timeout=5)

            if response.status_code == 401:
                log.error("You are not authorized, invalid token")
                raise AuthenticationError('You are not authorized, invalid token')
            ...
            if response.status_code == 409:
                ...
                raise FileExistsError('f"Файл {cloud_path} уже существует пропускаем"')
            response.raise_for_status()

            upload_url = response.json()["href"]
            with open(local_path, "rb") as file:
                upload_response = requests.put(upload_url, files={"file": file}, timeout=5)
        ...
        try:
            ...
            with open(local_path, "rb") as file:
                response = requests.put(
                    reload_url, files={"file": file}, headers=self.headers, timeout=5
                )
                response.raise_for_status()
    ...
    def delete(self, filename):
        ...
        try:
            response = requests.delete(url_delete, headers=self.headers, params=params, timeout=5)
            if response.status_code == 401:
                log.error("You are not authorized, invalid token")
                raise AuthenticationError('You are not authorized, invalid token')
            response.raise_for_status()
    ...
    def check_folder(self):
        url = f"{self.base_url + '?path=' + self.dir_name}"
